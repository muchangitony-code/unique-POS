// Runtime route source injected by scripts/build.cjs.
...
async function resolveMpesaCallback(checkoutRequestId, resultCode, resultDesc, callbackBody) {
  return db.transaction(async (tx) => {
    ...
    if (['success','failed','cancelled','timeout'].includes(found.status)) return { stored:true, status:found.status };
    ...
    const paid = Math.min(Number(found.amount_kes), balance);
    ...
    if (!duplicate && paid > 0) await tx.execute(mpesaSql`INSERT INTO invoice_payments (invoice_id, amount, method, reference, notes, created_at) VALUES (${found.invoice_id}, ${paid}, 'mpesa', ${String(receipt).slice(0,255)}, ${'Daraja STK Push checkout ' + checkoutRequestId}, now())`);
    const newPaid = Number(invoice.amount_paid || 0) + (duplicate ? 0 : paid);
    ...
    return { stored:true, status:'success', receipt, amount:paid };
  });
}

__MPESA_ROUTER__.post("/invoices/:id/mpesa/stk-push", requireAuth, async (req, res) => {
  ...
  try {
    ...
    const invoice = invoiceRows[0];
    if (!invoice || !isBranchInScope(req, invoice.branch_id)) return res.status(404).json({ error:'Invoice not found.' });
...
__MPESA_ROUTER__.post("/public/mpesa/callback", async (req, res) => {
  try {
    ...
    const resolved = await resolveMpesaCallback(callback.CheckoutRequestID, Number(callback.ResultCode), callback.ResultDesc, req.body);
...
__MPESA_ROUTER__.get("/invoices/:id/mpesa/status", requireAuth, async (req, res) => {
  ...
  const invoiceRows = mpesaRows(await db.execute(mpesaSql`SELECT branch_id FROM invoices WHERE id=${id} LIMIT 1`));
  if (!invoiceRows[0] || !isBranchInScope(req, invoiceRows[0].branch_id)) return res.status(404).json({ error:'Invoice not found.' });
  const rows = mpesaRows(await db.execute(mpesaSql`SELECT checkout_request_id, status, amount_kes, result_code, result_desc, mpesa_receipt_number FROM mpesa_transactions WHERE invoice_id=${id} AND checkout_request_id=${checkout} LIMIT 1`));
...
__MPESA_ROUTER__.post("/invoices/:id/mpesa/query", requireAuth, async (req, res) => {
