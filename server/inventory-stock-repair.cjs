...
async function repairInventoryStock() {
  ...
  try {
    ...
    try {
      ...
      const exists = await client.query(`SELECT to_regclass('public.inventory_products_v2') AS products, to_regclass('public.inventory_stock_v2') AS stock, to_regclass('public.product_stock') AS legacy_stock, to_regclass('public.products') AS legacy_products, to_regclass('public.branches') AS branches`);
      const tables = exists.rows[0];
      if (!tables.products || !tables.stock || !tables.branches) {
        await client.query('ROLLBACK');
        return { repaired: 0, source: 'unavailable' };
      }
      const branch = await client.query(`SELECT id FROM branches WHERE is_active=TRUE AND (code='MAIN' OR lower(trim(name))='main branch') ORDER BY CASE WHEN code='MAIN' THEN 0 ELSE 1 END,id LIMIT 1`);
      if (!branch.rowCount) {
        await client.query('ROLLBACK');
        return { repaired: 0, source: 'no-main-branch' };
      }
        ...
        const result = await client.query(`
          WITH source_stock AS (
            SELECT lower(trim(p.product_code)) AS sku_key, lower(trim(p.product_name)) AS name_key,
                   ps.current_stock::numeric AS quantity, COALESCE(ps.branch_id,$1) AS branch_id
            ...
            FROM source_stock ss JOIN inventory_products_v2 ip
              ON lower(trim(ip.sku))=ss.sku_key OR (ss.sku_key IS NULL AND lower(trim(ip.name))=ss.name_key)
          ...
          const result = await client.query(`
            WITH source_rows AS (
              SELECT lower(trim(COALESCE(normalized_data->>'product_code',normalized_data->>'sku',raw_data->>'Product Code',raw_data->>'SKU'))) AS sku_key,
                     ...
                     GREATEST(COALESCE((SELECT CASE WHEN trim(e.value) ~ '^-?[0-9]+([.][0-9]+)?$' THEN trim(e.value)::numeric END FROM jsonb_each_text(COALESCE(normalized_data,'{}'::jsonb)||COALESCE(raw_data,'{}'::jsonb)) e WHERE regexp_replace(lower(e.key),'[^a-z0-9]','','g') IN ('currentstock','openingstock','availableqty','availablequantity','stock','qty','quantity') LIMIT 1),0),0) AS quantity
              FROM product_import_rows
            ), matched AS (
              SELECT ip.id AS product_id,MAX(sr.quantity) AS quantity FROM source_rows sr JOIN inventory_products_v2 ip ON (sr.sku_key IS NOT NULL AND lower(trim(ip.sku))=sr.sku_key) OR (sr.sku_key IS NULL AND sr.name_key IS NOT NULL AND lower(trim(ip.name))=sr.name_key) WHERE sr.quantity>0 GROUP BY ip.id
            )
            INSERT INTO inventory_stock_v2(product_id,branch_id,quantity_on_hand)
      ...
      // The production catalogue was created from a known workbook whose opening
      // quantities are preserved in this authoritative restoration script. The
      // normal migration runner is disabled in production, so execute this one
      // idempotent recovery only when no retained database source could restore stock.
      if (repaired===0) {
        ...
        if (fs.existsSync(recoveryPath)) {
          ...
          const count=await client.query(`SELECT COUNT(*)::int AS lines FROM inventory_stock_v2 WHERE branch_id=$1 AND quantity_on_hand>0`,[branchId]);
          repaired=Number(count.rows[0]?.lines||0);
          if (repaired>0) source='authoritative-opening-stock';
        }
      }
