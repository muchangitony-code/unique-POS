...
function mountInventoryV3(app) {
  ...
  app.get('/api/v3/inventory/dashboard', async (_req, res) => {
    try {
      const result = await db().query(`
        SELECT
  ...
  app.post('/api/v3/inventory/products', async (req, res) => {
    ...
    if (!values[0] || !values[2]) return res.status(400).json({ error: 'SKU and product name are required.' });
    try {
  ...
  app.patch('/api/v3/inventory/products/:id', async (req, res) => {
    ...
    values.push(id);
    try {
      const result = await db().query(`UPDATE inventory_products_v2 SET ${fields.join(',')}, updated_at=NOW() WHERE id=$${values.length} AND is_active=TRUE RETURNING *`, values);
      if (!result.rowCount) return res.status(404).json({ error: 'Product not found.' });
      res.set('Cache-Control', 'no-store').json({ product: result.rows[0] });
    } catch (error) {
      if (error.code === '23505') return res.status(409).json({ error: 'SKU or barcode already exists.' });
      res.status(400).json({ error: error.message });
    }
  ...
  app.delete('/api/v3/inventory/products/:id', async (req, res) => {
    ...
    try {
      await withTransaction(async (client) => {
        const result = await client.query('DELETE FROM inventory_products_v2 WHERE id=$1 RETURNING id', [id]);
        if (!result.rowCount) {
      ...
      console.error('[inventory-v3] delete failed', error);
      res.status(400).json({ error: error.message });
    }
  });
