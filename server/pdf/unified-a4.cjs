...
async function logoBuffer(value){
  ...
  try {
    if(/^data:image\/(png|jpeg|jpg);base64,/i.test(raw)) {
      const b=Buffer.from(raw.split(',')[1],'base64'); return b.length<=MAX_LOGO_BYTES?b:null;
    }
    let filename=raw;
    if(/^https?:\/\//i.test(raw)) {
      const r=await fetch(raw,{redirect:'follow'}); if(!r.ok) return null;
      const type=String(r.headers.get('content-type')||'').toLowerCase(); if(!/^image\/(png|jpeg|jpg)$/i.test(type)) return null;
      const b=Buffer.from(await r.arrayBuffer()); return b.length<=MAX_LOGO_BYTES?b:null;
    }
...
function customerBox(pdf,data,y){
  const h=62; pdf.fillColor(COLORS.soft).strokeColor(COLORS.line).lineWidth(.6).roundedRect(M,y,W,h,4).fillAndStroke();
  text(pdf,'CUSTOMER',M+12,y+9,{font:'bold',size:7.5,color:COLORS.accent});
  text(pdf,data.customer.name||'Walk-in Customer',M+12,y+24,{font:'bold',size:10.5,width:W-24});
  ...
  if(details) text(pdf,details,M+12,y+41,{size:7.2,color:COLORS.muted,width:W-24});
  return y+h;
}
...
async function renderUnifiedA4({type,doc:input,company}){
  ...
  const headerData=()=>{const h=header(pdf,data);const cy=customerBox(pdf,data,h+14);return tableHeader(pdf,cy+14)+4;};
  const bottom=FOOTER_Y-18; const totalReserve=112+(data.notes?50+height(pdf,data.notes,W-20,8):0)+(data.terms?50+height(pdf,data.terms,W-20,8):0);
  const pages=[]; let current=[]; let used=0; const firstY=headerData();
  pdf.flushPages();
  const probe=new PDFDocument({size:'A4'}); registerFonts(probe); let available=bottom-firstY;
  for(const item of data.items){const h=rowHeight(probe,item);if(current.length&&used+h>available){pages.push(current);current=[];used=0;}current.push(item);used+=h;}
  if(current.length||!pages.length) pages.push(current);
  for(let pi=0;pi<pages.length;pi++){
    pdf.addPage({size:'A4',margins:{top:M,bottom:M,left:M,right:M}}); let y=headerData(); pages[pi].forEach((item,i)=>{y=row(pdf,item,y,i);});
    if(pi===pages.length-1){let by=y+14;const notesH=data.notes?Math.max(42,height(pdf,data.notes,W-20,8)+31):0;const termsH=data.terms?Math.max(42,height(pdf,data.terms,W-20,8)+31):0;const contentH=100+(data.notes?8+notesH:0)+(data.terms?8+termsH:0);const ty=Math.min(by,bottom-contentH);totalsBox(pdf,t,data.currency,ty);let iy=ty+108;if(data.notes){iy+=infoBlock(pdf,'Notes',data.notes,iy)+8;}if(data.terms)infoBlock(pdf,type==='invoice'?'Terms & Conditions':'Quotation Terms',data.terms,iy);}
  }
