...
function normalizeLogo(value) {
  ...
  if (source.startsWith('data:image/')) {
    ...
    if (comma > 0) { try { const b = Buffer.from(source.slice(comma + 1), 'base64'); if (b.length > 4) return b; } catch (_) {} }
  }
  return source;
}
function normalizeDocument(type, doc, company) {
  const logo = normalizeLogo(company.logo ?? company.logoUrl ?? company.logo_url) || BRAND.logo;
  return {
    ...
    company: { name: BRAND.legalName, tagline: BRAND.tagline, address: BRAND.address, phone: BRAND.phone, website: BRAND.website, email: str(company.email), taxId: str(company.taxId), logo }
  };
}
function validateDocument(type, doc, company) {
  ...
  str(doc.number, 'number', true); dateLike(doc.date, 'date', true); type === 'invoice' ? dateLike(doc.dueDate, 'dueDate') : dateLike(doc.validUntil, 'validUntil'); normalizeCustomer(doc.customer);
...
module.exports = { validateDocument, normalizeDocument, normalizeLogo };
