...
function detectDocumentType(payload) {
  ...
  const meta = objectFirst(root.meta, root.metadata, root.documentMeta, root.document_metadata, source.meta, source.metadata);
  const rawType = first(root.type, root.documentType, root.document_type, root.docType, root.kind, source.type, source.documentType, source.document_type, source.docType, source.kind, meta.type, meta.documentType);
  const normalized = normalizeType(rawType);
  if (normalized.includes('receipt') || normalized.includes('sale') || root.receipt || root.receiptId || root.saleId) return 'receipt';
  if (normalized.includes('quotation') || normalized.includes('quote') || root.quotation || root.quotationId || root.quotationNumber || root.quotation_number) return 'quotation';
  if (normalized.includes('invoice') || root.invoice || root.invoiceId || root.invoiceNumber || root.invoice_number) return 'invoice';
  return '';
}

function adaptDocumentPayload(payload, paper = 'a4') {
  const root = payload && typeof payload === 'object' ? payload : {};
  const source = objectFirst(root.doc, root.document, root.invoice, root.quotation, root);
  const meta = objectFirst(root.meta, root.metadata, root.documentMeta, root.document_metadata, source.meta, source.metadata);
  const type = detectDocumentType(root);
  if (!type) throw new Error(`Unsupported PDF document type: ${String(root.type || root.documentType || '(missing)')}`);
...
module.exports = { adaptDocumentPayload, detectDocumentType };
