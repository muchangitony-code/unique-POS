...
function text(pdf, value, x, y, width, o = {}) { pdf.font(o.font || 'body').fontSize(o.size || FONT).fillColor(o.color || C.ink).text(String(value ?? ''), x, y, { width, align: o.align || 'left', lineGap: 0 }); }
...
function drawThermalLogo(pdf, svg, width, y) { if (!svg) throw new Error('Thermal branding logo could not be loaded'); const size = Math.min(82, width - 20); const x = (width - size) / 2; try { SVGtoPDF(pdf, svg, x, y, { width: size, height: size, preserveAspectRatio: 'xMidYMid meet' }); return y + size + 6; } catch (error) { throw new Error(`Thermal branding logo render failed: ${error?.message || error}`); } }
function measureReceipt(data, company, paper) {
  const width = WIDTHS[paper] || WIDTHS['80mm']; const inner = width - MARGIN * 2; const probe = new PDFDocument({ size: [width, 2000], margins: { top: MARGIN, bottom: MARGIN, left: MARGIN, right: MARGIN } }); registerFonts(probe);
  let y = MARGIN + 8 + Math.min(82, width - 20) + 6 + 16 + 11 + 10 + 10 + 12 + 8 + 17 + 11 + 11 + 15 + 7 + 12;
  const items = Array.isArray(data.items) ? data.items : [];
  for (const item of items) { const desc = String(item.description || 'Item'); const h = Math.max(18, probe.heightOfString(desc, { width: inner * .50, font: 'body', fontSize: FONT }) + 4); y += h; }
  y += 7 + 13 * 3 + 34 + 15;
  if (data.notes) y += 4 + probe.heightOfString(String(data.notes), { width: inner, font: 'body', fontSize: 7.5 }) + 5;
  y += 6 + 14;
  probe.end();
  return Math.ceil(y + 24);
}

async function renderReceiptDocument({ doc: data, company, paper = '80mm' }) {
  const width = WIDTHS[paper] || WIDTHS['80mm']; const height = measureReceipt(data, company, paper);
  const items = Array.isArray(data.items) ? data.items : [];
  ...
  const pdf = new PDFDocument({ size: [width, height], margins: { top: MARGIN, bottom: MARGIN, left: MARGIN, right: MARGIN }, autoFirstPage: true, info: { Title: `Receipt ${data.number}`, Author: BRAND.legalName } });
  registerFonts(pdf);
  const chunks = []; const result = new Promise((resolve, reject) => { pdf.on('data', c => chunks.push(c)); pdf.once('end', () => resolve(Buffer.concat(chunks))); pdf.once('error', reject); });
  let y = MARGIN; const inner = width - MARGIN * 2;
  pdf.fillColor(C.ink).rect(0, 0, width, 3).fill(); pdf.fillColor(C.orange).rect(width * .72, 0, width * .28, 3).fill(); y += 8;
  ...
  text(pdf, `No: ${data.number}`, MARGIN, y, inner, { size: 8 }); y += 11; text(pdf, `Date: ${data.date || '—'}`, MARGIN, y, inner, { size: 8 }); y += 11; text(pdf, `Customer: ${data.customer?.name || 'Walk-in Customer'}`, MARGIN, y, inner, { size: 8 }); y += 15;
  ...
  for (const item of items) { const total = lineTotal(item).total; const desc = String(item.description || 'Item'); const h = Math.max(18, pdf.heightOfString(desc, { width: inner * .50, font: 'body', fontSize: FONT }) + 4); text(pdf, desc, MARGIN, y, inner * .50, { size: FONT }); text(pdf, formatNumber(item.qty), MARGIN + inner * .50, y, inner * .14, { size: FONT, align: 'right' }); text(pdf, money(total, data.currency), MARGIN + inner * .64, y, inner * .36, { size: FONT, align: 'right' }); y += h; }
  pdf.strokeColor(C.line).moveTo(MARGIN, y).lineTo(width - MARGIN, y).stroke(); y += 7;
  for (const [label, value] of [['Subtotal', totals.subtotal], ['Discount', -totals.discount], ['VAT', totals.tax]]) { text(pdf, label, MARGIN, y, inner * .50, { size: FONT, color: C.muted }); text(pdf, money(value, data.currency), MARGIN + inner * .50, y, inner * .50, { size: FONT, align: 'right' }); y += 13; }
  ...
  if (data.notes) { y += 4; text(pdf, data.notes, MARGIN, y, inner, { size: 7.5, color: C.muted }); y += pdf.heightOfString(String(data.notes), { width: inner, font: 'body', fontSize: 7.5 }) + 5; }
