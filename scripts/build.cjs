...
function patchA4RendererLogoSupport() {
  ...
  if (!source.includes("require('../branding.config.cjs')")) {
    ...
    source = source.replace(marker, marker + "const BRAND = require('../branding.config.cjs');\n");
  }
  ...
  if (svgData) { try { const decoded = Buffer.from(svgData[1], 'base64'); const svg = svgText(decoded); if (svg) return svg; } catch (_) {} }
  if (/^(iVBOR|\\/9j\\/)/.test(raw)) { try { const buffer = Buffer.from(raw, 'base64'); if (buffer.length <= MAX_LOGO && imageBuffer(buffer)) return buffer; } catch (_) {} }
  const candidates = [];
  if (raw.startsWith('/')) { candidates.push(path.join(process.cwd(), 'public', raw.replace(/^\\/+/, ''))); candidates.push(path.join(process.cwd(), raw.replace(/^\\/+/, ''))); }
  else if (!/^https?:\\/\\//i.test(raw)) { candidates.push(path.join(process.cwd(), raw)); candidates.push(path.join(process.cwd(), 'public', raw)); }
  for (const filename of candidates) { try { const buffer = fs.readFileSync(filename); if (buffer.length > MAX_LOGO) continue; if (imageBuffer(buffer)) return buffer; const svg = svgText(buffer); if (svg) return svg; } catch (_) {} }
  if (!/^https?:\\/\\//i.test(raw)) return null;
  try { const response = await fetch(raw, { redirect: 'follow' }); if (!response.ok) return null; const buffer = Buffer.from(await response.arrayBuffer()); if (buffer.length > MAX_LOGO) return null; if (imageBuffer(buffer)) return buffer; return svgText(buffer); } catch (_) { return null; }
}`;
  ...
  if (!buffer) console.warn('[pdf] Branding logo missing; document is not customer-ready');
  if (typeof buffer === 'string' && /^\\s*<svg(?:\\s|>)/i.test(buffer)) {
    try { const SVGtoPDF = require('@leduard/svg-to-pdfkit'); SVGtoPDF(pdf, buffer, x + 4, y + 4, { width: size - 8, height: size - 8, preserveAspectRatio: 'xMidYMid meet' }); }
    catch (error) { console.warn('[pdf] Branding SVG logo render failed:', error?.message || error); }
  } else if (buffer) {
    try { pdf.image(buffer, x + 4, y + 4, { fit: [size - 8, size - 8], align: 'center', valign: 'center' }); }
    catch (error) { console.warn('[pdf] Branding raster logo render failed:', error?.message || error); }
  }
  pdf.restore();
}`;
  ...
  if (source.includes(metaMarker) && !source.includes('doc.company.website,\n    doc.company.address')) source = source.replace(metaMarker, '    doc.company.website,\n    doc.company.address,\n    [doc.company.phone, doc.company.email].filter(Boolean).join(\'  ·  \'),');
  fs.writeFileSync(rendererPath, source, 'utf8');
  console.log('[build] shared A4 branding support installed');
}
...
function buildRuntimeBundle() {
  ...
  try { withUserManagement = patchUserManagementRoutes(source); }
  catch (error) {
    const message = String(error && error.message || error);
    if (/Express app (not found|not found)|could not locate Express application variable|application listen point not found/i.test(message)) console.warn(`[build] User-management runtime patch skipped: ${message}`);
    else throw error;
  }
  ...
  if (!runtime.includes('USER_MANAGEMENT_PATCH_V2') && withUserManagement !== source) throw new Error('Build verification: user-management patch was expected but is absent from runtime bundle');
  if (!runtime.includes("require('./server/pdf/index.cjs')") || !runtime.includes("require('./server/pdf/document-adapter.cjs')")) throw new Error('Build verification: unified A4 PDF renderer was not installed in runtime bundle');
  if (runtime.includes('renderLegacyDocumentPdf') || runtime.includes('legacyRenderPdfBuffer') || runtime.includes('legacy-adapter.cjs')) throw new Error('Build verification: obsolete A4 PDF renderer is still present in runtime bundle');
}
