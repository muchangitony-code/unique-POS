...
const { patchBulkImportRoutes } = require('./bulk-import-server.cjs');
...
function patchDocumentDeletionRoutes(source) {
  ...
  const deleteRoute = [`${router}.delete("/invoices/:id", requireAuth, requireSuperAdmin, async (req, res) => {`,'  const id = parseInt(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id, 10);','  const [invoice] = await db.select().from(invoicesTable).where(eq(invoicesTable.id, id));','  if (!invoice || !isBranchInScope(req, invoice.branchId)) { res.status(404).json({ error: "Invoice not found" }); return; }','  if (["partial", "paid"].includes(invoice.status)) { res.status(409).json({ error: "Paid or partially paid invoices cannot be permanently deleted. Void or reverse the transaction instead." }); return; }','  const payments = await db.select().from(invoicePaymentsTable).where(eq(invoicePaymentsTable.invoiceId, id));','  if (payments.length) { res.status(409).json({ error: "This invoice has payment records and cannot be permanently deleted." }); return; }','  const { reason } = req.body ?? {};','  const items = await db.select().from(invoiceItemsTable).where(eq(invoiceItemsTable.invoiceId, id));','  if (invoice.status !== "draft") { for (const item of items) { const product = await db.select().from(productsTable).where(eq(productsTable.id, item.productId)); if (product.length) { const { before, after } = await adjustBranchStock(invoice.branchId, item.productId, (b) => b + item.quantity); await db.insert(stockMovementsTable).values({ branchId: invoice.branchId, productId: item.productId, type: "adjustment", quantity: item.quantity, quantityBefore: before, quantityAfter: after, reference: invoice.invoiceNumber, notes: `Stock restored after deletion of invoice ${invoice.invoiceNumber}` }); } } }','  await db.delete(invoicePaymentsTable).where(eq(invoicePaymentsTable.invoiceId, id));','  await db.delete(invoiceItemsTable).where(eq(invoiceItemsTable.invoiceId, id));','  await db.delete(invoicesTable).where(eq(invoicesTable.id, id));','  await logAudit(req, { action: "invoice.deleted", entityType: "invoice", entityId: id, description: `Permanently deleted invoice ${invoice.invoiceNumber}${reason ? " — Reason: " + reason : ""}`, metadata: { invoice: invoice.invoiceNumber, status: invoice.status, reason: reason ?? null } });','  res.sendStatus(204);','});',''].join('\n');
...
function buildRuntime() {
  ...
  source = patchDocumentDeletionRoutes(source); source = patchQuotationEditRoutes(source); source = installPdfEngine(source); source = patchMpesaRoutes(source); source = patchBulkImportRoutes(source); fs.writeFileSync(runtimeBundle, source, 'utf8');
  ...
  if (!runtime.includes('BULK_IMPORT_V2_PATCH') || !runtime.includes("/api/healthz")) throw new Error('Build: fresh Bulk Import V2 / health patch missing from runtime bundle');
...
function verifyFiles() {
  const required = ['app.js','index.cjs','index.runtime.cjs','public/index.html','public/app.js','public/invoices.js','public/mpesa.js','public/bulk-import-v2.js','server/bulk-import-v2.cjs','server/bulk-import-v2-router.cjs','scripts/bulk-import-server.cjs','server/document-branding.cjs','public/assets/branding/logo.svg','public/assets/branding/logo-monochrome.svg','server/pdf/index.cjs','server/pdf/a4-renderer.cjs','server/pdf/receipt.cjs','server/pdf/schema.cjs','server/pdf/format.js','server/pdf/fonts.cjs','server/pdf/document-adapter.cjs','server/services/mpesa/runtime-routes.cjs','scripts/user-management-server.cjs','scripts/pdf-fixtures.js','scripts/pdf-receipt-smoke.cjs','scripts/pdf-production-regression.cjs','tests/pdf-branding-integrity.cjs','tests/pdf-logo-propagation.cjs','tests/pdf-document-smoke.cjs','tests/pdf-structural.cjs','assets/fonts/DejaVuSans.ttf','assets/fonts/DejaVuSans-Bold.ttf','assets/fonts/LICENSE.txt'];
  for (const file of required) if (!fs.existsSync(path.join(root,file))) throw new Error(`Build: missing required file ${file}`);
  for (const stale of ['scripts/build-safe.cjs','scripts/build-guard.cjs','server/branding.config.cjs','scripts/README-invoice-fix.txt','public/receipt-logo-fix.js','public/receipt-print-payment-fix.js','public/invoice-workspace.js']) if (fs.existsSync(path.join(root,stale))) throw new Error(`Build: stale document workaround still exists: ${stale}`);
  const renderer=fs.readFileSync(path.join(root,'server/pdf/a4-renderer.cjs'),'utf8'); if(!renderer.includes("require('../document-branding.cjs')"))throw new Error('Build: A4 renderer is not using shared document branding'); if(renderer.includes('const C = {'))throw new Error('Build: A4 renderer contains a private palette'); if(!renderer.includes('loadLogo(BRAND.logo)'))throw new Error('Build: canonical logo fallback missing'); if(!renderer.includes('refusing to generate an unbranded customer document'))throw new Error('Build: A4 renderer must refuse unbranded output');
...
console.log('[build] authoritative invoice, quotation, receipt and fresh bulk-import runtime verified');
