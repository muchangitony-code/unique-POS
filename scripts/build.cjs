...
const {isRootPdfRendererImport}=require(path.join(root,'server/pdf/bundle-loader.cjs'));const fake={filename:path.join(root,'index.cjs')};for(const form of ['./server/pdf/a4-renderer','./server/pdf/a4-renderer.cjs'])if(!isRootPdfRendererImport(form,fake,path.join(root,'index.cjs')))throw new Error(`Build: quotation PDF interception missing for ${form}`);
