...
for(const script of ['build-runtime-patch.cjs','patch-pos-product-access.cjs']){const patch=spawnSync(process.execPath,[path.join(__dirname,script)],{stdio:'inherit'});if(patch.status!==0)process.exit(patch.status||1);}
