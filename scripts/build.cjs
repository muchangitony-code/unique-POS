...
function installUnifiedPdfEngine(source) {
  ...
  const { renderUnifiedA4 } = require('./server/pdf/unified-a4.cjs');
  ...
  if (adapted.type === 'invoice' || adapted.type === 'quotation') {
    return renderUnifiedA4({ type: adapted.type, doc: adapted.doc, company: adapted.company });
  }
...
function patchDocumentDeletionRoutes(source) {
  ...
  const invoiceRouter = invoicePayMatch[1]; const invoicePayRoute = invoicePayMatch[0];
...
function buildRuntimeBundle() {
  ...
  if (!runtime.includes("require('./server/pdf/unified-a4.cjs')") || !runtime.includes("require('./server/pdf/document-adapter.cjs')")) throw new Error('Build verification: clean unified A4 PDF renderer was not installed in runtime bundle');
...
const requiredFiles = [
  ...
  'server/pdf/index.cjs', 'server/pdf/unified-a4.cjs', 'server/pdf/receipt.cjs', 'server/pdf/schema.cjs', 'server/pdf/format.js', 'server/pdf/fonts.cjs', 'server/pdf/bundle-loader.cjs', 'server/pdf/document-adapter.cjs',
