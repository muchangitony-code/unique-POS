...
const { patchBranchScopedInventoryCatalogue } = require('./inventory-v3-runtime-patch.cjs');
...
function buildRuntime() {
  ...
  source = patchDocumentDeletionRoutes(source); source = patchQuotationEditRoutes(source); source = installPdfEngine(source); source = patchMpesaRoutes(source); source = patchBranchScopedInventoryCatalogue(source); fs.writeFileSync(runtimeBundle, source, 'utf8');
...
function verifyFiles() {
  const required = ['app.js', 'index.cjs', 'index.runtime.cjs', 'public/index.html', 'public/app.js', 'public/invoices.js', 'public/mpesa.js', 'server/document-branding.cjs', 'public/assets/branding/logo.svg', 'public/assets/branding/logo-monochrome.svg', 'server/pdf/index.cjs', 'server/pdf/a4-renderer.cjs', 'server/pdf/receipt.cjs', 'server/pdf/schema.cjs', 'server/pdf/format.js', 'server/pdf/fonts.cjs', 'server/pdf/document-adapter.cjs', 'server/services/mpesa/runtime-routes.cjs', 'scripts/user-management-server.cjs', 'scripts/pdf-fixtures.js', 'scripts/pdf-receipt-smoke.cjs', 'scripts/pdf-production-regression.cjs', 'tests/pdf-branding-integrity.cjs', 'tests/pdf-logo-propagation.cjs', 'tests/pdf-document-smoke.cjs', 'tests/pdf-structural.cjs', 'assets/fonts/DejaVuSans.ttf', 'assets/fonts/DejaVuSans-Bold.ttf', 'assets/fonts/LICENSE.txt'];
