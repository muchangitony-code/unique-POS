...
function findFunctionEnd(source, start) {
  ...
  if (open < 0) throw new Error('Build: function opening brace not found');
...
function installPdfEngine(source) {
  ...
  if (start < 0) throw new Error('Build: could not locate application PDF entrypoint');
...
function patchDocumentDeletionRoutes(source) {
  ...
  if (!quotationMatch) throw new Error('Build: quotation DELETE route not found');
  ...
  const payMatch = patched.match(/\b(router[A-Za-z0-9_$]*)\.post\("\/invoices\/:id\/pay",\s*async\s*\(req,\s*res\)\s*=>\s*\{/);
  if (!payMatch) throw new Error('Build: invoice payment route not found');
  const router = payMatch[1];
  const deleteRoute = [
...
function patchMpesaRoutes(source) {
  ...
  if (!match) throw new Error('Build: document preview router not found for M-Pesa routes');
  const routeTemplate = path.join(root, 'server', 'services', 'mpesa', 'runtime-routes.cjs');
  if (!fs.existsSync(routeTemplate)) throw new Error(`Build: missing ${routeTemplate}`);
...
function buildRuntime() {
  if (!fs.existsSync(sourceBundle)) throw new Error(`Build: missing ${sourceBundle}`);
  let source = fs.readFileSync(sourceBundle, 'utf8');
  try { source = patchUserManagementRoutes(source); }
  catch (error) {
    const message = String(error?.message || error);
    if (/Express app .*not found|could not locate Express application variable|application listen point not found/i.test(message)) console.warn(`[build] User-management patch skipped: ${message}`);
    else throw error;
  }
  source = patchDocumentDeletionRoutes(source);
  source = installPdfEngine(source);
  source = patchMpesaRoutes(source);
  ...
  for (const stale of ['patch-pdf-renderer', 'PDF_SVG_LOGO_PATCH', 'renderLegacyDocumentPdf', 'legacyRenderPdfBuffer', 'legacy-adapter.cjs', 'server/pdf/stable.cjs']) {
    if (runtime.includes(stale)) throw new Error(`Build: stale PDF renderer reference remains in runtime: ${stale}`);
  }
}

function verifyFiles() {
  const required = [
    'app.js', 'index.cjs', 'index.runtime.cjs', 'product-bulk.cjs', 'public/index.html', 'public/app.js', 'public/mpesa.js',
    'server/branding.config.cjs', 'public/assets/branding/logo.svg', 'public/assets/branding/logo-monochrome.svg',
    'server/pdf/index.cjs', 'server/pdf/a4-renderer.cjs', 'server/pdf/receipt.cjs', 'server/pdf/schema.cjs', 'server/pdf/format.js', 'server/pdf/fonts.cjs', 'server/pdf/document-adapter.cjs',
    'server/services/mpesa/runtime-routes.cjs', 'scripts/user-management-server.cjs', 'scripts/pdf-fixtures.js', 'scripts/pdf-receipt-smoke.cjs',
    'assets/fonts/DejaVuSans.ttf', 'assets/fonts/DejaVuSans-Bold.ttf', 'assets/fonts/LICENSE.txt'
  ];
  for (const file of required) if (!fs.existsSync(path.join(root, file))) throw new Error(`Build: missing required file ${file}`);
  const source = fs.readFileSync(path.join(root, 'server/pdf/a4-renderer.cjs'), 'utf8');
  if (!source.includes("require('../branding.config.cjs')")) throw new Error('Build: A4 renderer is not connected to shared branding config');
  if (source.includes('const C = {')) throw new Error('Build: A4 renderer contains a private color palette');
  if (!source.includes('loadLogo(BRAND.logo)')) throw new Error('Build: A4 renderer has no canonical logo fallback');
}

for (const font of fonts) {
  if (!fs.existsSync(font) || !fs.statSync(font).isFile() || fs.statSync(font).size === 0) throw new Error(`Build: invalid PDF font ${font}`);
}
...
const checkFiles = [
  'server/branding.config.cjs', 'server/pdf/index.cjs', 'server/pdf/a4-renderer.cjs', 'server/pdf/receipt.cjs', 'server/pdf/schema.cjs', 'server/pdf/document-adapter.cjs',
  'scripts/user-management-server.cjs', 'scripts/pdf-fixtures.js', 'index.runtime.cjs'
];
for (const file of checkFiles) {
  const result = spawnSync(process.execPath, ['--check', path.join(root, file)], { stdio: 'inherit' });
  if (result.status !== 0) process.exit(result.status || 1);
}
console.log('[build] authoritative document engine and runtime bundle verified');
