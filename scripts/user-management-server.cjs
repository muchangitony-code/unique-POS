...
function patchUserManagementRoutes(source) {
  ...
  const code = [
    ...
    '      if(!name || !email) return res.status(400).json({error:"Name and email are required"});',
    '      if(!roles.has(role)) return res.status(400).json({error:"Invalid user role"});',
    '      if(password && password.length < 10) return res.status(400).json({error:"Password must contain at least 10 characters"});',
    ...
    '      if(password){',
    '        const hash=await bcryptAdmin.hash(password,12);',
    '        const updated=await db.execute(sql`UPDATE users SET name=${name},email=${email},phone=${phone || null},role=${role},branch_id=${branchId || null},is_active=${active},password_hash=${hash},password_changed_at=now(),failed_login_attempts=0,locked_until=NULL WHERE id=${id} RETURNING id,name,email,phone,role,branch_id AS "branchId",is_active AS "isActive"`);',
    '        const user=resultRows(updated)[0];',
    '        if(typeof logAudit === "function") await logAudit(req,{action:"user.updated",entityType:"user",entityId:id,description:"Updated user and password",metadata:{role,active,passwordChanged:true}});',
    '        return res.json({user:safeUser(user),passwordChanged:true});',
    '      }',
    ...
    '      const user=resultRows(updated)[0];',
    '      if(typeof logAudit === "function") await logAudit(req,{action:"user.updated",entityType:"user",entityId:id,description:"Updated user",metadata:{role,active,passwordChanged:false}});',
    '      return res.json({user:safeUser(user),passwordChanged:false});',
