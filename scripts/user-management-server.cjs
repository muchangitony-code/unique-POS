 ...
 * Supports both source and esbuild-generated Express app forms.
 */
function patchUserManagementRoutes(source) {
  ...
  const injected = [
    ...
    "      const supplied = String(req.body?.password || '').trim(); const password = supplied || randomPassword(); if (password.length < 10) return res.status(400).json({ error: 'Password must contain at least 10 characters' });",
    ...
    "      await logAudit(req, { action:'user.password_reset', entityType:'user', entityId:id, description:'Reset user password', metadata:{ generated: !supplied } });",
