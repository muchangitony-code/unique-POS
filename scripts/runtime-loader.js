...
const RUNTIME_MOUNT_MARKER = 'UNIQUEPOS_RUNTIME_MOUNTS_CLEAN_V2';
...
function loadIndex() {
  ...
  const sourceFilename = fs.existsSync(runtimeFilename)
    ? runtimeFilename
    : path.join(__dirname, '..', '..', 'index.cjs');

  const source = prepareRuntimeSource(sourceFilename);
  // If prepareRuntimeSource didn't change anything, require the original file directly
  if (source === fs.readFileSync(sourceFilename, 'utf8')) {
