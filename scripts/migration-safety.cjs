...
function classifyMigration(sqlText) {
  ...
  for (const statement of statements) {
    ...
    if (/^TRUNCATE\b/.test(upper)) findings.push('TRUNCATE');
    if (/^DELETE\s+FROM\b/.test(upper)) findings.push('DELETE');
    if (/^DROP\s+(TABLE|SCHEMA|DATABASE|TYPE|SEQUENCE|VIEW|MATERIALIZED\s+VIEW)\b/.test(upper)) findings.push('DROP');
    if (/^ALTER\s+TABLE\b/.test(upper) && /\bDROP\s+(COLUMN|CONSTRAINT)\b/.test(upper)) findings.push('ALTER TABLE DROP');
    if (/\bCASCADE\b/.test(upper) && (/^TRUNCATE\b/.test(upper) || /^DELETE\s+FROM\b/.test(upper) || /^DROP\b/.test(upper) || /\bDROP\s+(COLUMN|CONSTRAINT)\b/.test(upper))) {
      findings.push('CASCADE');
    }
  }
...
function requireDeploymentCredential() {
  ...
  if (token.length < 32) {
    throw new Error('Migration deployment blocked: MIGRATION_DEPLOY_TOKEN must be a distinct 32+ character deployment secret.');
  }
  if (process.env.MIGRATION_CONFIRMATION !== CONFIRMATION) {
    throw new Error(`Migration deployment blocked: set MIGRATION_CONFIRMATION=${CONFIRMATION}.`);
  }
}

function requireBackupFile(filePath) {
  if (!filePath) {
    throw new Error('Destructive migration blocked: MIGRATION_BACKUP_FILE is required and must point to a fresh pg_dump backup.');
  }
  ...
  if (Date.now() - stat.mtimeMs > maxAge) {
    throw new Error(`Destructive migration blocked: backup is older than ${Math.round(maxAge / 60000)} minutes: ${resolved}`);
  }
  return resolved;
}

function requireDestructiveApproval(names) {
  if (process.env.MIGRATION_ALLOW_DESTRUCTIVE !== 'YES') {
    throw new Error(`Destructive migration blocked. Explicitly set MIGRATION_ALLOW_DESTRUCTIVE=YES after written approval for: ${names.join(', ')}`);
  }
  if (process.env.MIGRATION_DESTRUCTIVE_APPROVAL !== DESTRUCTIVE_APPROVAL) {
    throw new Error(`Destructive migration blocked: set MIGRATION_DESTRUCTIVE_APPROVAL=${DESTRUCTIVE_APPROVAL} only for the specifically approved migration set.`);
  }
  ...
  if (missing.length) {
    throw new Error(`Destructive migration blocked: explicit approval is missing for ${missing.join(', ')}.`);
  }
}
...
async function assertMigrationDeploymentSafe(options = {}) {
  ...
  if (retiredPending.length) {
    throw new Error(`Migration deployment blocked: retired migration(s) are still pending: ${retiredPending.map((item) => item.name).join(', ')}. Audit the target database before deployment; these historical data-cleanup migrations must never run automatically.`);
  }
...
module.exports = {
  ...
  scanMigrations,
  auditMigrationState,
  assertMigrationDeploymentSafe,
  requireBackupFile
};

if (require.main === module) {
  auditMigrationState().then((rows) => {
    console.table(rows.map(({ name, destructive, findings, policy, status }) => ({
      name,
      status,
      destructive,
    ...
    const pendingDestructive = rows.filter((row) => row.status === 'pending' && row.destructive);
    if (pendingDestructive.length) process.exitCode = 2;
  }).catch((error) => {
