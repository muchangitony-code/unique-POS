...
// Patch the checkout validation once. Railway may execute this script during
// both build and startup, so it must be idempotent. The current POS has a real
// completeSale() function; completeSaleAndThen() is not present in this build,
// so it is optional rather than a build-breaking requirement.
if (!source.includes(CHECKOUT_MARKER)) {
  function patchCheckoutFunction(functionName, required) {
    const start = source.indexOf(`async function ${functionName}()`);
    if (start < 0) {
      if (required) throw new Error(`Checkout function not found: ${functionName}`);
      console.log(`[checkout-fix] Optional function ${functionName} not present; skipping.`);
      return;
    }
...
console.log("UniquePOS checkout repaired: basket sales automatically populate Cash Received with the live Grand Total; manual positive amounts remain respected and credit sales remain zero-paid.");
