...
if (!fs.existsSync(path.join(__dirname, '..', 'server', 'pdf', 'fonts.generated.cjs'))) { const r = spawnSync(process.execPath, [path.join(__dirname, 'build.cjs')], { stdio: 'inherit' }); if (r.status) process.exit(r.status); }
...
function base(type, overrides = {}) { return { type, doc: { number: type === 'invoice' ? 'INV-0024' : 'QUO-0024', date: '2026-08-16', dueDate: '2026-08-30', validUntil: '2026-08-30', customer: { name: 'Acme Electrical Supplies', address: 'Industrial Area, Nairobi', phone: '+254 711 111 111', email: 'buyer@example.test', taxId: 'P051111111B' }, items: [{ description: 'Solar cable 6mm²', qty: 2, unitPrice: '2500.00', taxRate: 16, discount: '0' }], currency: 'KES', notes: 'Thank you for your business.', terms: 'Payment is due according to the stated date.', ...overrides }, company }; }
...
(async () => {
  ...
  console.log('[pdf-fixtures] Generated 7 PDFs in out/');
