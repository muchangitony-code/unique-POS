...
function requireDeploymentCredential() {
  ...
  if (process.env.MIGRATION_CONFIRMATION !== 'APPLY_MIGRATIONS') {
    throw new Error('Migration deployment blocked: MIGRATION_CONFIRMATION=APPLY_MIGRATIONS is required.');
  }
}

function requireDestructiveApproval(names) {
  if (process.env.MIGRATION_ALLOW_DESTRUCTIVE !== 'YES') {
    throw new Error(`Destructive migration blocked. Written approval is required before setting MIGRATION_ALLOW_DESTRUCTIVE=YES for: ${names.join(', ')}`);
  }
  if (process.env.MIGRATION_DESTRUCTIVE_APPROVAL !== DESTRUCTIVE_APPROVAL) {
    throw new Error(`Destructive migration blocked: MIGRATION_DESTRUCTIVE_APPROVAL=${DESTRUCTIVE_APPROVAL} is required.`);
  }
...
async function main() {
  ...
  const retiredPending = rows.filter((row) => row.status === 'pending' && row.policy === 'retired');
  if (retiredPending.length) {
    throw new Error(`Migration deployment blocked: retired historical migration(s) are pending: ${retiredPending.map((row) => row.name).join(', ')}. Run the migration audit against the target database before proceeding.`);
  }
