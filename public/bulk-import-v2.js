(function () {
  ...
  function importProducts() {
    ...
    var rows = p.rows.filter(function (r) { return !r.errors.length; }).map(function (r) { return { rowNumber: r.rowNumber, normalized: r.normalized }; });
    ...
    fetch('/api/v2/products/bulk-import', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ branch_id: branchId, rows: rows }) }).then(function (response) { return response.json().then(function (data) { if (!response.ok) throw new Error(data.error || 'Import failed'); return data; }); }).then(function (data) { toast('Import complete: ' + data.created + ' created, ' + data.updated + ' updated.', 'success'); state.preview = null; state.file = null; document.getElementById('bulkV2File').value = ''; document.getElementById('bulkV2Status').textContent = 'Import complete. Upload another catalogue when ready.'; document.getElementById('bulkV2Mapping').innerHTML = ''; document.getElementById('bulkV2Preview').innerHTML = ''; }).catch(function (error) { button.disabled = false; button.textContent = 'Import Valid Products'; toast(error.message, 'error'); });
  }
