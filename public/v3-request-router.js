...
(()=>{
  ...
  window.fetch=async function(input,init){
    ...
    if(method.toUpperCase()==='GET'&&u.origin===location.origin&&u.pathname==='/api/products'){
      ...
      const r=await nativeFetch(target.toString(),{credentials:'same-origin',cache:'no-store'});
      if(!r.ok)return r;
    ...
    if(method.toUpperCase()==='GET'&&u.origin===location.origin&&u.pathname==='/api/inventory/stock-count'){
      ...
      const r=await nativeFetch(target.toString(),{credentials:'same-origin',cache:'no-store'});if(!r.ok)return r;
