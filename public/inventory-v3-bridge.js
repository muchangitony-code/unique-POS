(() => {
  ...
  async function loadProducts() {
    const response = await fetch('/api/v3/inventory/products?q=' + encodeURIComponent(query), { headers: { Accept: 'application/json' } });
    if (!response.ok) throw new Error('New inventory service is unavailable.');
  ...
  function productRows() {
    if (!products.length) return '<tr><td colspan="8" class="empty-state">No products. This new catalogue is intentionally empty and contains no legacy stock.</td></tr>';
    return products.map(p => `<tr>
      ...
      <td>${Number(p.quantity_on_hand || 0).toLocaleString()}</td>
      <td><button class="btn btn-outline" type="button" data-v3-edit="${esc(p.id)}">Edit</button></td>
    </tr>`).join('');
  }
  ...
  function render() {
    ...
    el.innerHTML = `<section class="card section-card inventory-v3-page">
      <div class="section-head"><div><h2>New Product & Inventory</h2><p>Clean catalogue — no legacy product data is used.</p></div>
      <button class="btn btn-primary" type="button" id="inventoryV3Add">Add Product</button></div>
      <div class="inline-group" style="margin-bottom:16px">
        <input id="inventoryV3Search" class="form-control" type="search" placeholder="Search name, SKU or barcode" value="${esc(query)}">
        <button class="btn btn-outline" type="button" id="inventoryV3Refresh">Refresh</button>
      </div>
      <div class="table-wrap"><table><thead><tr><th>SKU</th><th>Barcode</th><th>Product</th><th>Category</th><th>Unit</th><th>Selling</th><th>Stock</th><th></th></tr></thead><tbody>${productRows()}</tbody></table></div>
      <div class="inventory-v3-status">${loaded ? `${products.length} products in the new catalogue` : 'Loading new catalogue…'}</div>
    </section>`;
  ...
  function showError(error) {
    const el = root();
    if (el) el.querySelector('.inventory-v3-status').textContent = error.message || 'Unable to load the new inventory.';
  }

  function form(product) {
    const p = product || {};
    return `<form id="inventoryV3Form" class="stack-form">
      <label>SKU<input name="sku" required value="${esc(p.sku)}"></label>
      ...
      <label>Unit<input name="unit" value="${esc(p.unit || 'pcs')}"></label>
      ...
      <label>Supplier<input name="supplier" value="${esc(p.supplier)}"></label>
  ...
  function showModal(title, body) {
    ...
    document.getElementById('modalSubtitle').textContent = 'New Product & Inventory module';
  ...
  function openCreate() {
    ...
    document.getElementById('inventoryV3Form').addEventListener('submit', async event => {
      ...
      const response = await fetch('/api/v3/inventory/products', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(body) });
      const data = await response.json();
  ...
  async function openEdit(id) {
    ...
    document.getElementById('inventoryV3Form').addEventListener('submit', async event => {
      event.preventDefault();
      const body = Object.fromEntries(new FormData(event.target).entries());
      const response = await fetch('/api/v3/inventory/products/' + encodeURIComponent(id), { method: 'PATCH', headers: {'Content-Type':'application/json'}, body: JSON.stringify(body) });
      const data = await response.json();
