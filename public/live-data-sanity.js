(function () {
  ...
  function setKpi(label, value, isMoney) {
    ...
    cards.forEach(function (card) {
      ...
      if (trend) trend.textContent = 'Updated from authoritative records';
    });
  }
  ...
  async function repairDashboard() {
    ...
    try {
      ...
      var branchId = Number(branchSelect && branchSelect.value) || 1;
      var results = await Promise.all([
        getJson('/api/dashboard/stats').catch(function () { return {}; }),
        getJson('/api/dashboard/sales-chart').catch(function () { return []; }),
        getJson('/api/v3/inventory/dashboard?branchId=' + encodeURIComponent(branchId)).catch(function () { return null; })
      ]);
      var stats = results[0] || {};
      var chart = results[1] || [];
      ...
      // Inventory KPIs must never come from the legacy dashboard aggregate.
      // The V3 inventory balance is the sole authority for stock counts.
      if (inventory) {
        setKpi('Low Stock Items', firstNumber(inventory.low_stock_items, 0), false);
        setKpi('Out of Stock Items', firstNumber(inventory.out_of_stock_items, 0), false);
      } else {
        setKpi('Low Stock Items', 0, false);
        setKpi('Out of Stock Items', 0, false);
      }
  ...
  function schedule() {
    clearTimeout(timer);
    timer = setTimeout(repairDashboard, 40);
  }
  ...
  // Keep the dashboard from being overwritten by delayed legacy rendering.
  setInterval(function () {
    if (route() === 'dashboard' && !busy) repairDashboard();
  }, 500);
  schedule();
})();
