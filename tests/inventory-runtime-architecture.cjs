...
if (!loader.includes('UNIQUEPOS_INVENTORY_V3_MOUNT_V2') || !loader.includes('mountInventoryV3BranchRoutes') || !loader.includes('mountInventoryV3')) {
...
if (!branchRoutes.includes("app.get('/api/v3/inventory/dashboard'") || !branchRoutes.includes('ON s.product_id = p.id AND s.branch_id = $1')) {
...
console.log('[inventory-architecture] PASS: Inventory V3 is mounted with branch-scoped catalogue/dashboard and without the deleted fragile patcher.');
