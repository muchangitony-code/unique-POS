...
/** Keep manual/imported codes intact while generating a consistent code for blank/auto-created products. */
function buildProductCode(categoryName: string | null | undefined, productName: string, existingCodes: string[]): string {
  const catPrefix = (String(categoryName || "GEN").trim() || "GEN")
    .replace(/[^A-Za-z0-9]/g, "")
    .slice(0, 3)
    .toUpperCase() || "GEN";
  const nameSlug = (String(productName || "PRODUCT").trim() || "PRODUCT")
    .replace(/[^A-Za-z0-9]+/g, "-")
    .replace(/^-|-$/g, "")
    .slice(0, 20)
    .toUpperCase() || "PRODUCT";
  ...
  return `${base}-${String(next).padStart(3, "0")}`.slice(0, 64);
}
