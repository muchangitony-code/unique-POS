...
function categoryPrefix(category: string | null | undefined): string {
  const compact = String(category || "").toUpperCase().replace(/[^A-Z0-9]/g, "");
  return (compact.slice(0, 4) || "PROD");
}

async function generateProductCode(categoryName: string | null | undefined): Promise<string> {
  const [row] = await db.execute(sql`SELECT nextval('public.inventory_product_identifier_seq')::BIGINT AS sequence`);
  const sequence = Number((row as { sequence: number | string }).sequence);
  return `${categoryPrefix(categoryName)}-${String(sequence).padStart(6, "0")}`;
}
...
router.post("/products", requireRole("administrator", "manager", "storekeeper"), async (req, res): Promise<void> => {
  ...
  const finalProductCode = (!requestedCode || requestedCode.toUpperCase() === "AUTO") ? await generateProductCode(category?.name) : requestedCode;
