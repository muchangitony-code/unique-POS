...
import { db, branchesTable, salesTable, invoicesTable, quotationsTable, purchasesTable, expensesTable, customersTable, suppliersTable, productStockTable, usersTable, stockMovementsTable, auditLogTable } from "@workspace/db";
...
async function fmt(b: typeof branchesTable.$inferSelect) {
  ...
  return { id: b.id, name: b.name, code: b.code, address: b.address, county: b.county, phone: b.phone, phone2: b.phone2, email: b.email, manager: b.manager, kra_pin: b.kraPin, paybill_number: b.paybillNumber, paybill_account: b.paybillAccount, till_number: b.tillNumber, bank_name: b.bankName, bank_account_name: b.bankAccountName, bank_account_number: b.bankAccountNumber, logo_url: logoUrl, receipt_footer: b.receiptFooter, invoice_footer: b.invoiceFooter, quotation_footer: b.quotationFooter, is_active: b.isActive, created_at: b.createdAt };
}

function mapBody(body: Record<string, unknown>): Record<string, unknown> {
  const map: Record<string, string> = { name: "name", code: "code", address: "address", county: "county", phone: "phone", phone2: "phone2", email: "email", manager: "manager", kra_pin: "kraPin", paybill_number: "paybillNumber", paybill_account: "paybillAccount", till_number: "tillNumber", bank_name: "bankName", bank_account_name: "bankAccountName", bank_account_number: "bankAccountNumber", logo_url: "logoUrl", receipt_footer: "receiptFooter", invoice_footer: "invoiceFooter", quotation_footer: "quotationFooter", is_active: "isActive" };
  const out: Record<string, unknown> = {}; for (const [k, col] of Object.entries(map)) if (body[k] !== undefined) out[col] = body[k]; return out;
}

router.get("/branches", async (req, res): Promise<void> => { const scope = getBranchScope(req); const rows = await db.select().from(branchesTable).orderBy(branchesTable.id); const visible = scope.isSuper ? rows : rows.filter((b) => b.id === scope.userBranchId); res.json(await Promise.all(visible.map(fmt))); });
router.get("/branches/options", async (_req, res): Promise<void> => { const rows = await db.select({ id: branchesTable.id, name: branchesTable.name, code: branchesTable.code, is_active: branchesTable.isActive }).from(branchesTable).orderBy(branchesTable.name); res.json(rows); });
