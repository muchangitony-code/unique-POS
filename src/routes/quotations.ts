...
function validateQuotationItems(items: unknown): string | null {
  if (!Array.isArray(items) || items.length === 0) return "items required";
  for (let i = 0; i < items.length; i++) {
    const item = items[i] as Partial<LineItemInput> | null | undefined;
    if (!item || typeof item !== "object") return `Invalid item at position ${i + 1}`;
    const qty = Number(item.quantity);
    const price = Number(item.unit_price);
    const discount = Number(item.discount ?? 0);
    const vat = Number(item.vat_rate ?? 16);
    if (!Number.isFinite(qty) || qty <= 0) return `Quantity must be greater than zero for item ${i + 1}`;
    if (!Number.isFinite(price) || price < 0) return `Unit price cannot be negative for item ${i + 1}`;
    if (!Number.isFinite(discount) || discount < 0 || discount > 100) return `Discount must be between 0 and 100% for item ${i + 1}`;
    if (!Number.isFinite(vat) || vat < 0) return `VAT rate cannot be negative for item ${i + 1}`;
    if (item.product_id == null) {
      const description = String(item.description ?? "").trim();
      if (!description || ["unknown", "undefined", "null", "item", "non-stock item"].includes(description.toLowerCase())) {
        return `Enter a description/name for non-stock item ${i + 1}`;
      }
    }
  }
  return null;
...
router.post("/quotations", async (req, res): Promise<void> => {
  ...
  const validationError = validateQuotationItems(items);
  ...
  for (const item of processedItems) await db.insert(quotationItemsTable).values({ quotationId: quotation.id, productId: item.product_id as any, description: item.description ?? null, unit: item.unit ?? null, quantity: item.quantity, unitPrice: item.unit_price.toString(), discount: item.discount.toString(), vatRate: item.vat_rate.toString(), total: item.total.toString() });
...
router.get("/quotations/:id", async (req, res): Promise<void> => { const id = parseInt(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id, 10); const [q] = await db.select().from(quotationsTable).where(eq(quotationsTable.id, id)); if (!q || !isBranchInScope(req, q.branchId)) { res.status(404).json({ error: "Quotation not found" }); return; } res.json(await formatQuotation(q)); });
router.patch("/quotations/:id", async (req, res): Promise<void> => { const id = parseInt(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id, 10); const { customer_id, status, notes, valid_until, delivery_time, warranty, payment_terms } = req.body; const [existing] = await db.select().from(quotationsTable).where(eq(quotationsTable.id, id)); if (!existing || !isBranchInScope(req, existing.branchId)) { res.status(404).json({ error: "Quotation not found" }); return; } const [q] = await db.update(quotationsTable).set({ customerId: customer_id, status, notes, validUntil: valid_until, deliveryTime: delivery_time, warranty, paymentTerms: payment_terms }).where(eq(quotationsTable.id, id)).returning(); res.json(await formatQuotation(q)); });
router.delete("/quotations/:id", async (req, res): Promise<void> => { const id = parseInt(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id, 10); const [existing] = await db.select().from(quotationsTable).where(eq(quotationsTable.id, id)); if (!existing || !isBranchInScope(req, existing.branchId)) { res.status(404).json({ error: "Quotation not found" }); return; } await db.delete(quotationItemsTable).where(eq(quotationItemsTable.quotationId, id)); await db.delete(quotationsTable).where(eq(quotationsTable.id, id)); res.sendStatus(204); });

router.post("/quotations/:id/convert", async (req, res): Promise<void> => {
  const id = parseInt(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id, 10); const [q] = await db.select().from(quotationsTable).where(eq(quotationsTable.id, id)); if (!q || !isBranchInScope(req, q.branchId)) { res.status(404).json({ error: "Quotation not found" }); return; } if (q.status === "converted") { res.status(409).json({ error: "Quotation already converted" }); return; }
  const invoiceNumber = await nextDocumentNumber("invoice"); let invoice: typeof invoicesTable.$inferSelect; let customerName: string | null = null;
  try {
    const result = await db.transaction(async (tx) => {
      const claimed = await tx.update(quotationsTable).set({ status: "converted" }).where(and(eq(quotationsTable.id, id), sql`${quotationsTable.status} <> 'converted'`)).returning(); if (!claimed.length) return null;
      ...
      for (const item of qItems) { await tx.insert(invoiceItemsTable).values({ invoiceId: inv.id, productId: item.productId as any, description: item.description, unit: item.unit, quantity: item.quantity, unitPrice: item.unitPrice, discount: item.discount, vatRate: item.vatRate, total: item.total }); const d = item.productId == null ? undefined : deducted.find((x) => x.product_id === item.productId); if (d) await tx.insert(stockMovementsTable).values({ branchId: q.branchId, productId: item.productId!, type: "sale", quantity: -item.quantity, quantityBefore: d.before, quantityAfter: d.after, reference: invoiceNumber, notes: `Invoice from ${q.quotationNumber}` }); }
      let cName: string | null = null; if (q.customerId) { if (Number(inv.total) > 0) await tx.update(customersTable).set({ balance: sql`${customersTable.balance} + ${Number(inv.total)}` }).where(eq(customersTable.id, q.customerId)); const [c] = await tx.select({ name: customersTable.name }).from(customersTable).where(eq(customersTable.id, q.customerId)); cName = c?.name ?? null; }
      return { inv, cName };
    });
    if (!result) { res.status(409).json({ error: "Quotation already converted" }); return; } invoice = result.inv; customerName = result.cName;
